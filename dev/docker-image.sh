...
bw_version=$(env $(cat .env | grep -v '#' | xargs) bash -c 'echo $BW_DEV_VERSION')
if [[ ! $bw_version ]]; then
    echoerr "BW_DEV_VERSION value must be specified in file'.env'"
    exit 1
fi
...
if [[ ! $build_only && $bw_version == $did_version ]]; then
    echoerr "BW_DEV_VERSION ($bw_version) in file'.env' must differ (be bigger) than version ($did_version) in file'$version_fspec' in line: image: bazawinner/$target-${bw_proj_name}-$service:$did_version"
    exit 1
fi

env $(cat .env | grep -v '#' | xargs) bash -c '\
    tag=bazawinner/'$target'-$BW_PROJ_NAME-'$service':$BW_DEV_VERSION
    echo Building $tag . . .
    docker build '${opts[@]}' \
        $(\
